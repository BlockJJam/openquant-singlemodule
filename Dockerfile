...
WORKDIR /home/quant
ENTRYPOINT ["java", "-jar","SpringApp.jar","-Dpring.profiles.active=${APPLICATION_YAML_PATH}","-Dspring-boot.run.arguments=--spring.datasource.url=${DATABASE_DEPLOY_URL} --spring.datasource.username=${DATABASE_DEPLOY_ID} --spring.datasource.password=${DATABASE_DEPLOY_PWD}"]
